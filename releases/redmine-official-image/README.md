...
This chart bootstraps a [Redmine](https://hub.docker.com/_/redmine?tab=description) deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
```bash
$ helm install --name my-release .../redmine
```
...
| `redmineDBmigrate`                  | If automatically migrate db on container starting | `true` |
| `redminePluginsMigrate`             | If automatically migrate plugin db on container starting | `true` |
...
| `externalDatabase.databaseName`     | DatabaseName                               | `redmine`                                               |
...
```bash
$ helm install --name my-release -f values.yaml .../redmine
```
